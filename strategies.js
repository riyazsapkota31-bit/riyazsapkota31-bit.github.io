 ...
 * FIXED: Data Gap & fileToPart undefined
 */

let API_KEY = localStorage.getItem('omni_api_v3') || "";
const MODEL = "gemini-1.5-flash-latest"; // Optimized stable model

window.onload = () => { if (API_KEY) lockUI(); };

// --- UI DYNAMICS: PROTECTS INPUT FIELD ---
function markFile(idx) {
    ...
    if (input.files && input.files[0]) {
        ...
        // Update content div only; preserves the <input> element's data
        content.innerHTML = `
...
async function executeScan() {
    ...
    // Explicit selection to avoid 'Data Gap' errors
    const files = [
    ...
    try {
        ...
        const prompt = `System: Expert Multi-Strategy Analyst (SMC + Price Action). 
        Analyze 4 charts for structural confluence. 
        1. If HTF/LTF/DXY disagree, return bias "WAIT". 
        2. If "WAIT", identify a "Watch Level" price in the logic.
...
function renderOutput(res, resultBox) {
    ...
    if (res.bias === "WAIT") {
        ...
        logicTxt.innerText = `WATCH LEVEL: ${res.support || "POI"} | ${res.logic}`;
